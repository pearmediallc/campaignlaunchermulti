...
router.post('/create', authenticate, requireFacebookAuth, refreshFacebookToken, requirePermission('campaign', 'create'), uploadSingle, parseFormDataJson, validateStrategyForAll, async (req, res) => {
  ...
  try {
    ...
    // This check is read-only and does NOT modify existing functionality
    // ============================================================================
    ...
    await SafetyChecks.logVerification(
      req.user.id,
      selectedAdAccountId,
      req.body.campaignName,
      verification,
      null // jobId will be set in Week 2 when job tracking is implemented
    );

    // Block request if verification failed
    if (!verification.canProceed) {
      console.log('❌ [Week 1 Safety] Pre-creation verification FAILED - blocking request');
    ...
    if (verification.warnings && verification.warnings.length > 0) {
      console.log('⚠️  [Week 1 Safety] Proceeding with warnings:');
      verification.warnings.forEach(w => console.log(`   - ${w}`));
    }

    console.log('✅ [Week 1 Safety] Pre-creation verification PASSED - proceeding with creation\n');
    // ============================================================================
    // END WEEK 1 ENHANCEMENT
    // ============================================================================

    // ============================================================================
    // WEEK 2-6 ENHANCEMENTS: JOB TRACKING + RETRY + VERIFICATION + ROLLBACK
    // ============================================================================
    ...
    // Create job and initialize slots
    ...
    console.log(`✅ [Week 2 Safety] Job created: ID ${job.id}`);

    // Initialize slots (pre-allocate for idempotency protection)
    await JobTracker.initializeSlots(job);
    console.log(`✅ [Week 2 Safety] Slots initialized`);

    // Mark job as started
    await JobTracker.markJobStarted(job);
    console.log(`✅ [Week 2 Safety] Job started\n`);

    // Wrap entire creation in try-catch for comprehensive error handling
    let safetySystemResult = null;
    try {
    // ============================================================================
    // END WEEK 2-6 SETUP
    // ============================================================================
    ...
    const campaignData = {
      ...
      // ============================================================================
      // FEATURE: DYNAMIC TEXT VARIATIONS & DYNAMIC CREATIVE
      // ============================================================================
      ...
      dynamicCreativeMediaPaths: req.body.dynamicCreativeMediaPaths || null
      // ============================================================================
    };
    ...
    console.log(`\n🚀 ===============================================`);
    console.log(`🚀 STRATEGY FOR-ALL BATCH CREATION MODE`);
    console.log(`🚀 ===============================================`);
    console.log(`📝 Creating campaign with ${totalAdSets} ad sets and ${totalAdSets} ads`);
    console.log(`📊 This will create everything in batch API calls (NO postId dependency)`);
    ...
    try {
      ...
      // ============================================================================
      console.log(`\n📋 [Week 2 Safety] Updating slot tracking with created entities...`);

      // Mark campaign slot as created
      if (campaignId) {
        await JobTracker.markEntityCreated(job, 'campaign', 1, {
          id: campaignId,
          name: campaignData.campaignName
        });

        // Update job with Facebook campaign ID
        await job.update({ facebookCampaignId: campaignId });

        console.log(`   ✅ Campaign slot marked (ID: ${campaignId})`);
      }

      // Mark ad set slots as created
      // Note: Batch service doesn't return individual IDs, so we'll fetch from Facebook
      if (adSetsCreated > 0) {
        try {
          ...
          const createdAdSets = adSetsResponse.data?.data || [];
          console.log(`   📊 Fetched ${createdAdSets.length} ad sets from Facebook for tracking`);

          // Mark each ad set slot
          for (let i = 0; i < createdAdSets.length && i < adSetsCreated; i++) {
            await JobTracker.markEntityCreated(job, 'ad_set', i + 1, {
              id: createdAdSets[i].id,
              name: createdAdSets[i].name
            });
          }

          console.log(`   ✅ ${createdAdSets.length} ad set slots marked as created`);

          // Mark ad slots (1 ad per ad set)
          for (let i = 0; i < adsCreated; i++) {
            const adSetId = createdAdSets[i]?.id;
            if (adSetId) {
              ...
              const createdAds = adsResponse.data?.data || [];
              if (createdAds.length > 0) {
                await JobTracker.markEntityCreated(job, 'ad', i + 1, {
                  id: createdAds[0].id,
                  name: createdAds[0].name
                });
              }
            }
          }

          console.log(`   ✅ ${adsCreated} ad slots marked as created`);

        } catch (trackingError) {
          console.warn(`   ⚠️  Could not fetch entity details for tracking: ${trackingError.message}`);
          // Don't fail the request if tracking fails
        }
      }

      console.log(`✅ [Week 2 Safety] Slot tracking updated\n`);
      ...
      if (campaignData.dynamicCreativeEnabled || campaignData.dynamicTextEnabled) {
        ...
        if (campaignData.primaryTextVariations?.length > 0) {
          console.log(`  ✅ Primary Text Variations: ${campaignData.primaryTextVariations.length} options`);
        }
        if (campaignData.headlineVariations?.length > 0) {
          console.log(`  ✅ Headline Variations: ${campaignData.headlineVariations.length} options`);
        }
      } else {
    ...
    // ============================================================================
    // WEEK 2-6: POST-CREATION VERIFICATION & JOB COMPLETION
    // ============================================================================

    // Week 4: Post-creation verification
    console.log(`\n🔍 [Week 4 Safety] Post-creation verification...`);
    if (result.campaign?.id) {
      const verificationResult = await PostCreationVerification.reconcileJobWithFacebook(job, userFacebookApi);

      console.log(`✅ [Week 4 Safety] Verification complete:`);
      console.log(`   Verified: ${verificationResult.verified}/${verificationResult.totalSlots}`);
      console.log(`   Missing: ${verificationResult.missing}/${verificationResult.totalSlots}`);

      if (verificationResult.discrepancies.length > 0) {
        console.log(`⚠️  [Week 4 Safety] ${verificationResult.discrepancies.length} discrepancies detected`);
      }
      ...
      safetySystemResult = {
        ...
        finalCounts: {
          adSets: result.totalAdSets,
          ads: result.totalAds
        }
      };
    }

    // Week 6: Mark job as completed
    console.log(`\n✅ [Week 6 Safety] Marking job ${job.id} as completed`);
    await JobTracker.markJobCompleted(job);

    // Get final progress summary
    const progress = await JobTracker.getJobProgress(job);
    console.log(`📊 [Week 6 Safety] Final progress:`);
    console.log(`   Ad Sets: ${progress.adSets.created}/${progress.adSets.requested}`);
    console.log(`   Ads: ${progress.ads.created}/${progress.ads.requested}`);
    console.log(`   Retries used: ${progress.retries.count}/${progress.retries.budget}`);

    // ============================================================================
    // END WEEK 2-6 COMPLETION
    // ============================================================================
    ...
    await AuditService.logRequest(req, 'strategyForAll.create', 'campaign', result.campaign?.id, 'success', null, {
      ...
      // Add safety system metadata
      safetySystem: {
        jobId: job.id,
        verified: safetySystemResult?.verification?.verified || 0,
        retriesUsed: progress.retries.count
      }
    });
    ...
    res.json({
      success: true,
      message: 'Strategy for-all campaign created successfully with enterprise safety protections',
      data: {
        phase: 'initial',
        campaign: result.campaign,
        adSet: result.adSet,
        ads: result.ads,
        adAccount: facebookAuth.selectedAdAccount,
        ...
        safetySystem: {
          ...
          verification: {
            passed: true,
            verified: safetySystemResult?.verification?.verified || 0,
            discrepancies: safetySystemResult?.verification?.discrepancies?.length || 0
          },
          retries: {
            used: progress.retries.count,
            budget: progress.retries.budget
          },
          finalCounts: {
            adSets: progress.adSets.created,
            ads: progress.ads.created
          }
        }
      }
    });
    ...
    // ============================================================================
    // WEEK 3-6: COMPREHENSIVE ERROR HANDLING & ROLLBACK
    // ============================================================================
  } catch (error) {
    console.error('❌ [Safety System] Strategy for-all creation error:', error);
    console.error('   Error type:', error.constructor.name);
    console.error('   Error message:', error.message);

    // Week 6: Handle job failure (decide retry vs rollback)
    if (job) {
      console.log(`\n🔄 [Week 6 Safety] Handling job failure...`);

      const failureAction = await JobTracker.handleJobFailure(job, error, userFacebookApi);

      console.log(`📋 [Week 6 Safety] Failure action: ${failureAction.action}`);

      if (failureAction.action === 'rollback') {
        console.log(`\n🔄 [Week 5 Safety] Initiating rollback...`);
        console.log(`   Reason: ${failureAction.reason}`);

        // Week 5: Execute rollback
        const rollbackResult = await RollbackManager.executeRollback(job, userFacebookApi, {
          userConfirmed: true,
          reason: failureAction.reason
        });

        console.log(`✅ [Week 5 Safety] Rollback complete`);
        console.log(`   Entities deleted: ${rollbackResult.entitiesDeleted}`);
        console.log(`   Failed to delete: ${rollbackResult.entitiesFailed}`);

        await AuditService.logRequest(req, 'strategyForAll.create', null, null, 'failure_rolled_back', error.message, {
          adAccountId: selectedAdAccountId,
          strategyType: 'for-all',
          safetySystem: {
            jobId: job.id,
            action: 'rollback',
            entitiesDeleted: rollbackResult.entitiesDeleted,
            reason: failureAction.reason
          }
        });
        ...
        return res.status(500).json({
          ...
          safetySystem: {
            ...
            rollback: {
              executed: true,
              entitiesDeleted: rollbackResult.entitiesDeleted,
              entitiesFailed: rollbackResult.entitiesFailed
            },
            message: 'Campaign creation failed. All created entities have been rolled back to maintain clean state.'
          }
        });

      } else if (failureAction.action === 'retry') {
        console.log(`\n🔄 [Week 3 Safety] Will retry (attempt ${failureAction.attempt}/${failureAction.attempt + failureAction.remaining})`);

        await AuditService.logRequest(req, 'strategyForAll.create', null, null, 'failure_will_retry', error.message, {
          adAccountId: selectedAdAccountId,
          strategyType: 'for-all',
          safetySystem: {
            jobId: job.id,
            action: 'retry',
            attempt: failureAction.attempt,
            remaining: failureAction.remaining
          }
        });
        ...
        return res.status(500).json({
          ...
          safetySystem: {
            ...
            attempt: failureAction.attempt,
            remaining: failureAction.remaining,
            message: `Campaign creation encountered an error. Retry ${failureAction.attempt} of ${failureAction.attempt + failureAction.remaining} will be attempted automatically.`
          }
        });
      }
    }

    // Fallback error response (if no job or other error)
